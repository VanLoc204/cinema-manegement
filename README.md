...
1.  Mở terminal tại thư mục gốc dự án và di chuyển vào thư mục `backend`:
    ...
    npm install
    ```
2.  Tạo file cấu hình môi trường `.env` nằm trong thư mục `backend/` với nội dung mẫu như sau:
    ```env
    PORT=5000
    MONGO_URI=mongodb://127.0.0.1:27017/cinema
    
    # Cấu hình bảo mật JWT
    JWT_SECRET=cinema_lux_secure_secret_key_2026
    
    # Cấu hình kết nối cổng thanh toán PayOS
    PAYOS_CLIENT_ID=438782ad-2acb-4caf-a06c-be9f1936e72a
    PAYOS_API_KEY=4a6f43b3-aeb4-4a85-8e70-aaae5332579c
    PAYOS_CHECKSUM_KEY=259b352aacd52ac8f208271d9bc682725f801ce0a5d590117027d1bd290f88de
    
    # Cấu hình gửi mail OTP bằng SMTP Gmail (Dùng mật khẩu ứng dụng)
    EMAIL_USER=hovanloc.lv@gmail.com
    EMAIL_PASS=bsclocuheotddtgi
    ```

> [!WARNING]
> *Lưu ý về bảo mật:* Bạn nên thay thế `JWT_SECRET` bằng chuỗi ký tự ngẫu nhiên của riêng bạn. Cấu hình PayOS bên trên đã được thiết lập sẵn môi trường thử nghiệm (Sandbox) để bạn có thể test quét mã QR và thanh toán ảo bằng tiền giả mà không tốn phí thật.

3.  Khởi động Server Backend ở chế độ phát triển (Development):
    ```powershell
    npm run dev
    ```
...
1.  Mở một terminal mới (song song với terminal chạy backend) và trỏ vào thư mục `frontend`:
    ...
    npm install
    ```
2.  Khởi động ứng dụng React trên máy chủ ảo Vite:
    ```powershell
    npm run dev
    ```
